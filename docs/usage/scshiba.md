...
``` bash
Usage: scShiba -i experiment.tsv -c config.txt
...
Please check the [Manual](../manual/diff_splicing_sc.md/#1-prepare-inputs) to find how to prepare the `Expermiment table` and `Config file`.
...
``` bash
usage: sc2junc.py [-h] -i EXPERIMENT -o OUT
...
``` bash
usage: scpsi.py [-h] [-p NUM_PROCESS] [-f FDR] [-d PSI] [-r REFERENCE] [-a ALTERNATIVE] [-m MINIMUM_READS] [--onlypsi] [--excel] junctions event output
